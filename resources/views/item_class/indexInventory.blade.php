    ...
    <div class="card card-custom gutter-b">
        <div class="card-header">
            <div class="card-title">
                Inventory Classification - &nbsp;<span class="text-primary">{{$category->name}}</span>
            </div>
            ...
            <div class="card-toolbar">
                <div class="btn-group" role="group" aria-label="Basic example">
                    ...
                    &nbsp;&nbsp;
                    <a href="{{route('categoryinventory.index')}}" class="btn btn-success"><i class="fa fa-arrow-left"></i></a>
                </div>
            </div>
            @actionEnd
        </div>
                ...
                <table class="table table-bordered table-hover display font-size-sm" style="margin-top: 13px !important; width: 100%;">
                    ...
                    @foreach($classifications as $key => $value)
                        <tr>
                            <td>{{($key+1)}}</td>
                            <td>
                                <a href="{{route('itemsinventory.inventory', ['category'=>$cat_id,'classification' =>$value->id])}}">
                            ...
                            <div class="modal fade" id="edit{{$value->id}}" tabindex="-1" role="basic" aria-hidden="true" data-backdrop="static" data-keyboard="false">
                                <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
                                    <div class="modal-content">
                                        ...
                                        <form method="post" action="{{route('item_classinventory.update')}}" >
                                            @csrf
                            ...
                            <td class="text-center">
                                ...
                                <a href="{{route('item_classinventory.delete',['id' => $value->id])}}" title="Delete" class="btn btn-sm btn-danger btn-icon btn-icon-md" onclick="return confirm('Delete Category?')"><i class="fa fa-trash"></i></a>
                            </td>
                        </tr>
                    @endforeach
                    @actionEnd
    ...
    <div class="modal fade" id="addEmployee" tabindex="-1" role="dialog" aria-labelledby="addEmployee" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
            <div class="modal-content">
                ...
                <form method="post" action="{{route('item_classinventory.store')}}" >
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <div class="form col-md-12">
