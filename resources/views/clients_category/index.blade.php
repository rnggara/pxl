        ...
        <div class="card-header">
            ...
            <div class="card-toolbar">
                <div class="btn-group" role="group" aria-label="Basic example">
                    <a href="{{route('marketing.client.index')}}" title="Client Category" class="btn btn-secondary"><i class="fa fa-backspace"></i>Client List</a>
                    &nbsp;&nbsp;
                    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#addEmployee"><i class="fa fa-plus"></i>Add Client Category</button>
                </div>
                <!--end::Button-->
            </div>
        </div>
        <div class="card-body">
            <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                <table class="table table-bordered table-hover display font-size-sm" style="margin-top: 13px !important; width: 100%;">
                    ...
                    <tbody>
                    @foreach($client_categories as $key => $value)
                        <tr>
                            <td>{{($key+1)}}</td>
                            <td><a href="#edit{{$value->id}}" data-toggle="modal" class="btn btn-sm btn-primary" title="Edit">{{$value->name}}</a></td>
                            ...
                            <td class="text-center">
                                <a href="{{route('cc.delete',['id' => $value->id])}}" title="Delete" class="btn btn-sm btn-danger btn-icon btn-icon-md" onclick="return confirm('Delete client category?')"><i class="fa fa-trash"></i></a>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
