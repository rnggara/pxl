        ...
        <div class="card-header">
            ...
            <div class="card-toolbar">
                <div class="btn-group" role="group" aria-label="Basic example">
                    <a href="{{route('policy.hse.category')}}" title="Policy Category" class="btn btn-info"><i class="fa fa-cogs"></i></a>
                    &nbsp;&nbsp;
                ...
                <table class="table table-bordered table-hover display font-size-sm" style="margin-top: 13px !important; width: 100%;">
                    ...
                    @foreach($policy as $key => $value)
                        <tr>
                            ...
                            <td style="width: 20%">
                                ...
                                @if(isset($detailID[$value->id_main]))
                                    ...
                                    @else
                                        ...
                                        <a href="{{route('policy.hse.detail.viewappr',['id' => $detailID[$value->id_main]])}}" class="btn btn-xs btn-link"><i class="fa fa-search"></i>&nbsp;{{$value->id_main}}/{{strtoupper(\Session::get('company_tag'))}}-POLICY/{{date('m/y',strtotime($detailDate[$value->id_main]))}}</a>
                                        @if(isset($detailRev[$value->id_main]))
                            ...
                            <td class="text-center"><a href="{{route('policy.hse.detail',['id' => $value->id_main])}}" class="btn btn-xs btn-link"><i class="fa fa-search"></i>{{$value->topic}}</a></td>
                            ...
                            <td data-priority=1 class="text-center">
                                <a href="{{route('policy.hse.delete',['id' => $value->id_main])}}" title="Delete" class="btn btn-sm btn-danger btn-icon btn-xs" onclick="return confirm('Delete policy?')"><i class="fa fa-trash"></i></a>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
    ...
    <div class="modal fade" id="addEmployee" tabindex="-1" role="dialog" aria-labelledby="addEmployee" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
            <div class="modal-content">
                ...
                <form method="post" action="{{route('policy.hse.store')}}" >
                    @csrf
