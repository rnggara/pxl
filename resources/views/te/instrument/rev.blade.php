        ...
        <div class="card-header">
            ...
            <div class="card-toolbar">
                <div class="btn-group" role="group" aria-label="Basic example">
                    <a href="{{route('te.instrument.index')}}" class="btn btn-success mr-2"><i class="fa fa-arrow-circle-left"></i> </a>
                </div>
                <!--end::Button-->
            </div>
        </div>
                ...
                <table class="table table-bordered table-hover display font-size-sm te_instrument_update" style="margin-top: 13px !important; width: 100%;">
                    ...
                    @foreach($revisions as $key => $value)
                        <tr>
                            ...
                            <td><a href="{{route('te.instrument.revision_detail',['id' => $value->id])}}" class="btn btn-link btn-xs"><i class="fa fa-search"></i>{{$value->name}}</a></td>
                            <td class="text-center">
                                @if($value->picture == ''|| $value->picture == null)
                                    No Picture
                                @else
                                    <img src="{{str_replace('public','public_html',asset('/media/te_instrument_update/')).'/'.$value->picture}}" class="img-responsive center-block" height="15%">
                                @endif
                            </td>
