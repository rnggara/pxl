        ...
        <div class="card-header">
            ...
            <div class="card-toolbar">
                <div class="btn-group" role="group" aria-label="Basic example">
                    <a href="{{route('te.testeq.revision')}}" class="btn btn-success btn-xs"><i class="fa fa-arrow-circle-left"></i></a>
                </div>
                <!--end::Button-->
            </div>
        </div>
        <div class="card-body">
            <form method="post" action="{{route('te.testeq.revision_approve')}}" id="formsubmit">
                @csrf
                    ...
                    <div class="col-md-6">
                        <label for="" class="col-form-label">
                            ...
                            @if($old->picture == null || $old->picture = '')
                                No Picture
                            @else
                                <img src="{{str_replace('public','public_html',asset('/media/te_testeq/')).'/'.$old->picture}}" class="img-responsive center-block" height="15%">
                            @endif

                            @if($new->picture != $old->picture)
                                <i class="fa fa-arrow-right"></i>
                                <img src="{{str_replace('public','public_html',asset('/media/te_testeq_update/')).'/'.$new->picture}}" class="img-responsive center-block" height="15%">
                            @endif
                        </label>
                    </div>
                </div>
