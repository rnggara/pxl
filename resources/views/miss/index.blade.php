        ...
        <div class="card-header">
            ...
            <div class="card-toolbar">
                ...
                <div class="btn-group" role="group" aria-label="Basic example">
                    <a href="{{route('miss.getview')}}" class="btn btn-primary"><i class="fa fa-plus"></i>Add Miss</a>
                </div>
                ...
                <table class="table table-bordered table-hover display font-size-sm" style="margin-top: 13px !important; width: 100%;">
                    ...
                    @foreach($nearmiss as $key => $value)
                        <tr>
                            ...
                            <td class="text-left"><a href="{{route('miss.nm_view',['id'=>$value->id])}}" class="btn btn-link"><i class="fa fa-search"></i>&nbsp;&nbsp;{{$value->title}}</a></td>
                            <td class="text-center">
                                {{substr(strip_tags($value->deskripsi),0,60) }}
                            </td>
                            <td class="text-center" width="15%">
                                @if($value->pict == null)
                                    <a href='{{route('miss.getviewphoto',['id' => $value->id,'status' => 'edit'])}}' title='Upload Image'
                                    class='btn btn-primary btn-sm btn-icon'><i class='fa fa-pencil-alt'></i></a>
                                @else
                                    <a class='fancybox' href='{{route('miss.getviewphoto',['id' => $value->id,'status' => 'edit'])}}' data-fancybox-group='gallery' title='{{$value->title}}'>
                                    <img src='{{str_replace('public','public_html',asset('/media/miss_attachment/'))}}/{{$value->pict}}' data-fancybox-group='gallery' alt height='40px'/></a>
                                    &nbsp; <a href='{{route('miss.getviewphoto',['id' => $value->id,'status' => 'edit'])}}' class='btn btn-primary btn-xs' title='Edit Photo'><i class='fa fa-pencil-alt'></i></a>
                                @endif
                            </td>
                            <td class="text-center">
                                <a href='{{route('miss.getview',['id' => $value->id,'status' => 'follow'])}}' class='btn btn-primary btn-xs' title='Insert Follow Up'><i class='fa fa-pencil-alt'></i></a>
                            </td>
                            <td class="text-center" width="15%">
                                @if($value->pict_follow == '' || $value->pict_follow == null)
                                    <a href='{{route('miss.getviewphoto',['id' => $value->id,'status' => 'follow'])}}' title='Upload Image' class='btn btn-primary btn-xs'><i class='fa fa-pencil-alt'></i></a>
                                @else
                                    <a href='' target='_new'>
                                        <img src='{{str_replace('public','public_html',asset('/media/miss_attachment/'))}}/{{$value->pict_follow}}' data-fancybox-group='gallery' alt height='40px'/></a>

                                    <a href='{{route('miss.getviewphoto',['id' => $value->id,'status' => 'follow'])}}' class='btn btn-primary btn-xs'><i class='fa fa-pencil-alt'></i></a>
                                @endif
                            </td>
                            {{--<td>--}}
                            ...
                            <td class="text-center">
                                @if((($value->approved == '' || $value->approved == null) && ($value->follow_up_task != '' || $value->follow_up_task != null)))
                                    <form action='{{route('miss.approval')}}' method='post'>
                                        @csrf
                                        ...
                                        <button type='submit' class='btn btn-success btn-xs btn-icon' name='app' value='APPROVE' onclick="return confirm('Approve miss?')">
                                        <i class='fa fa-check'></i></button> |
                                        <button type='submit' class='btn btn-danger btn-xs btn-icon' name='app' value='DENIED' onclick="return confirm('Denied miss?')" >
                            ...
                            <td width="10%" class="text-center">
                                &nbsp;&nbsp;&nbsp;
                                <form action='{{route('miss.delete')}}' method='post'>
                                    @csrf
