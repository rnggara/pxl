...
@section('content')
    @if(session()->has('message_needsec_sallist_fail'))
        <div class="alert alert-danger">
            {{ session()->get('message_needsec_sallist_fail') }}
        </div>
    @endif
    @if(session()->has('message_needsec_sallist_success'))
        <div class="alert alert-success">
            {{ session()->get('message_needsec_sallist_success') }}
        </div>
    @endif
    <div class="card card-custom gutter-b">
        ...
        <div class="card-body">
            ...
            <form action='{{route('salarylist.submitNeedsec')}}' method='POST' class='col-md-4'>
                @csrf
