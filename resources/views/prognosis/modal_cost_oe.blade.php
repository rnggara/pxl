...
<form method="POST" action="{{route('marketing.prognosis.whitelists', $type)}}">
    @csrf
    <div class="modal-body">
        {{--PO--}}
        <div class="row">
        ...
        <div class="row">
            <div class="col-md-12">
                <h4 class="text-center">BID PERFORMA BOND</h4>
                @for($i=0; $i < 5; $i++)
                    <div class="row mb-5">
                        <div class="col-md-12">
                            <select name="bp[{{$i}}]" class="form-control select2" id="">
                                <option value="">Empty</option>
                                @foreach($prj_show as $item)
                                    <option value="{{$item->id}}" {{(isset($whitelist['bp'][$i]) && $item->id == $whitelist['bp'][$i]) ? "SELECTED" : ""}}>{{$item->prj_name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
