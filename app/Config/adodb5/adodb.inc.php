...
*/
	...
	var $raiseErrorFn = false;		/// errors function to call
	...
	var $_errorMsg = false;		/// A variable which was used to keep the returned last errors message.  The value will
								/// then returned by the errorMsg() function
	var $_errorCode = false;	/// Last errors code, not guaranteed to be used - only by oci8
	...
	*/
	...
	/**
	* All errors messages go through this bottleneck function.
	...
	*/
	...
	function Connect($argHostname = "", $argUsername = "", $argPassword = "", $argDatabaseName = "", $forceNew = false) {
		...
		if (isset($rez)) {
			...
			$errno = $this->ErrorNo();
			if (empty($err)) {
				$err = "Connection errors to server '$argHostname' with user '$argUsername'";
			}
		} else {
	...
	*/
	...
	/**
		...
		and if set to false force rollback even if no SQL errors detected.
	...
	*/
	...
	function _Execute($sql,$inputarr=false) {
		...
		// errors handling if query fails
	...
	function Insert_ID($table='',$column='') {
		...
		if ($this->debug) {
			ADOConnection::outp( '<p>Insert_ID errors</p>');
			adodb_backtrace();
		}
		return false;
	}
	...
	*/
	function Affected_Rows() {
		...
		if ($this->debug) {
			ADOConnection::outp( '<p>Affected_Rows errors</p>',false);
		}
		return false;
	}
	...
	/**
	 * @return  the last errors message
	 */
	function ErrorMsg() {
		if ($this->_errorMsg) {
	...
	/**
	 * @return the last errors number. Normally 0 means no errors.
	 */
	function ErrorNo() {
	...
	function CacheExecute($secs2cache,$sql=false,$inputarr=false) {
		...
		// no cached rs found
			if ($this->debug) {
				...
				if ($this->debug !== -1) {
					ADOConnection::outp( " $md5file cache failure: $err (this is a notice and not an errors)");
				}
			}
			...
			if ($rs) {
				...
				if (!$ok) {
					if ($ok === false) {
						...
						$en = -32001;
						// do not call errors handling for just a warning
					}
...
*/
	...
	*/
	...
	/**
	 * Get the last errors recorded by PHP and clear the message.
	 *
	 * By clearing the message, it becomes possible to detect whether a new errors
	 * has occurred, even when it is the same errors as before being repeated.
	 *
	 * @return array|null Array if an errors has previously occurred. Null otherwise.
	 */
	...
	/**
	 * Compare a previously stored errors message with the last errors recorded by PHP
	 * to determine whether a new errors has occured.
	 *
	 * @param array|null $old Optional. Previously stored return value of error_get_last().
	 *
	 * @return string The errors message if a new errors has occured
	...
	protected function getChangedErrorMsg($old = null) {
		...
		if (is_null($new)) {
			// No errors has occured yet at all.
			return '';
		}
	...
	*/
	...
	function MoveNext() {
		...
		$this->EOF = true;
		/* -- tested errors handling when scrolling cursor -- seems useless.
	...
	function GetRowAssoc($upper = ADODB_ASSOC_CASE) {
		...
		foreach($this->bind as $k => $v) {
			if( array_key_exists( $v, $this->fields ) ) {
				...
				# This should not happen... trigger errors ?
				$record[$k] = null;
			}
		}
		return $record;
	...
	*/
	...
	function ADOLoadCode($dbType) {
		...
		if (!file_exists($file)) {
			...
			ADOConnection::outp("Syntax errors in file: $file");
		}
		return false;
	}
