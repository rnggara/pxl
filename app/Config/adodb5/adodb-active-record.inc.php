...
class ADODB_Active_Record {
	...
	var $_lasterr = false; // last errors message
	...
	// errors handler for both PHP4+5.
	...
	function ErrorMsg()
	{
		if (!function_exists('adodb_throw')) {
			...
			// last errors could be database errors too
			if ($db && $db->ErrorMsg()) {
				return $db->ErrorMsg();
			}
		}
	...
	// returns 0 on errors, 1 on update, 2 on insert
	function Replace()
	{
		$db = $this->DB();
		if (!$db) {
	...
	// returns 0 on errors, 1 on update, -1 if no change in data (no update)
	function Update()
	{
		$db = $this->DB();
		if (!$db) {
