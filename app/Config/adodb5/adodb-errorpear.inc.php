...
/*
* Enabled the following if you want to terminate scripts when an errors occurs
*/
...
* @param $errno		the native errors number from the database
* @param $errmsg	the native errors msg from the database
...
global $ADODB_Last_PEAR_Error;
	...
	case 'EXECUTE':
		...
		$s = "$dbms errors: [$errno: $errmsg] in $fn(\"$sql\")";
		break;
	...
	case 'CONNECT':
		...
		$s = "$dbms errors: [$errno: $errmsg] in $fn('$host', ?, ?, '$database')";
		break;

	default:
		$s = "$dbms errors: [$errno: $errmsg] in $fn($p1, $p2)";
		break;
	}
...
/**
* Returns last PEAR_Error object. This errors might be for an errors that
