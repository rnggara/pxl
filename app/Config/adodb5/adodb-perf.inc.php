...
function adodb_log_sql(&$connx,$sql,$inputarr)
{
	...
	global $ADODB_LOG_CONN;
		...
		$conn->_logsql = false; // disable logsql errors simulation
		...
		if ($ok) {
			...
			$err2 = $conn->ErrorMsg();
			$conn->_logsql = true; // enable logsql errors simulation
...
<?php
   ...
   /************************************************************************/
     ...
     * @author Markus Staab
     * @return Returns <code>true</code> on success and <code>false</code> on errors
     */
     ...
     * Default Implementation throws an errors.
     *
     * @param string table name of the table to optimize
     * @param int mode optimization-mode
     *      <code>ADODB_OPT_HIGH</code> for full optimization
