...
class TeSlicklineController extends Controller
{
    function index(){
        $swt = Te_slickline::where('company_id', Session::get('company_id'))->get();
        return view('te.slickline.index', [
            'swt' => $swt
        ]);
    }

    function add(Request $request){
        $swt = new Te_slickline();
    ...
    function find($id){
        $swt = Te_slickline::find($id);

        return view('te.slickline._edit', [
            'swt_edit' => $swt
        ]);
    }

    function update(Request $request){
        $swt = Te_slickline::find($request->id_swt);
        $swt->subject = $request->project_name;
        if ($swt->save()){
            return redirect()->back();
        }
    ...
    function items($id){
        ...
        return view('te.slickline.items', [
            'swt' => $swt
        ]);
    }
    ...
    function update_items(Request $request){
        $swt = Te_slickline::find($request->id_pd);
