...
class MOMController extends Controller
{
        ...
        foreach ($mtg_absence as $key => $value){
            ...
            if (!empty($value->sig_address)) {
                if (strpos($value->sig_address,'signature')){
                    $thumbnail = "<div class='d-flex align-item-center'><div class='symbol symbol-90 mr-3'><img src='".str_replace('public','public_html',asset('/media/sign_mom/')).'/'.$value->sig_address."' class='img-responsive center-block' height='15%'></div></div>";
                } else {
                    $thumbnail = "<div class='d-flex align-item-center'><div class='symbol symbol-90 mr-3'><img src='".str_replace('public','public_html',asset('/media/sign_mom/')).'/'.$value->sig_address."' class='img-responsive center-block' height='15%'></div></div>";
                }
            }
    ...
    public function getMom($id){
        $mtg_mom = Mtg_mom::where('id_main', $id)
    ...
    public function getMtgAjax(){
        $mtgs = Mtg_main::where('company_id',\Session::get('company_id'))
        ...
        foreach ($mtgs as $key =>$value){
            ...
            $meeting['action'] = ($value->progress == 'created') ? "<a class='btn btn-success btn-icon btn-xs' href='".route('mom.action.progress',['id' => $value->id_main])."' onclick='return confirm(\"Are you sure?\")'><i class='fa fa-check-square'></i></a>" : "<a class='btn btn-icon btn-xs btn-primary' href='".route('mom.printMv', $value->id_main)."'><i class='fa fa-download'></i></a>";
...
//            'moms' =>$mom,
    ...
    public function signatureSave(Request $request){
        if (isset($request['imageData'])) {
            ...
            $folderPath = public_path("media/sign_mom/");
